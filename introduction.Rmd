...
As outlined by @r4ds in Figure \@ref(fig:workflow), most data science workflows require lots of iteration between data manipulation, visualization, and modeling. Acheiving these tasks through a programming language like `R` offers the opportunity to scale, automate, document, track provenance, and reproduce that workflow, but it usually comes at the cost of increasing the amount of cognitive load involved relative to a GUI-based system.^[For more on the benefits of using code over a GUI to perform data analysis, see @data-science-gui.] `R` packages like the **tidyverse** have been incredibly successful due to their ability to limit cognitive load without removing the benefits of performing analysis via code. Moreover, the **tidyverse**'s unifying principles of designing for humans, consistency, and composabilty makes iteration within and between these stages seamless -- an important but often overlooked challenge in exploratory data analysis (EDA) [@tidy-principles].

```{r workflow, echo = FALSE, fig.cap="The stages of a data science workflow by @r4ds.", out.width = "80%"}
...
Generating interactive graphics with code allows you to leverage all the benefits of a code-based workflow, while still acheiving tasks that are difficult to accomplish with code alone. For a simple example, if you were to visualize engine displacement (`displ`) versus miles per gallon (`hwy`) using the `mpg` dataset, you might wonder: "what are these cars with an unusually high value of `hwy` given their `displ`?" Rather than trying to write code to query those observations, it would be more easier and intuitive to draw an outline around the points to query the data behind them.
...
Figure \@ref(fig:mpg-lasso) demonstrates how we can transform that static **ggplot2** scatterplot into an interactive version that can be used to query the relevant observations (shown in another view). The framework that enables this kind of linked graphic is discussed in depth inside Chapter \@ref(graphical-queries), but the point here is that the added effort required to enable such functionality is relatively small. This is important, because although interactivity _can_ augment exploratory analysis, they are only _practical_ when we can iterate quickly. That's because, in a true exploratory setting, you have to make lots of visualizations before stumbling across something valuable. As an added bonus, when you find that valuable insight, since the result is a web-based graphic, it can be easily shared with others. Moreover, since these interactive graphcis are based on the **htmlwidgets** framework, they work seamlessly inside of larger **rmarkdown** documents, inside **shiny** apps, `RStudio`, `Jupyter` notebooks, the `R` prompt, and more.
...
Now, from Figure \@ref(fig:mpg-lasso) we've learned that most of these unusual points can be explained by a simple feature of the data (`model == 'corvette'`). When it comes to presenting this data and finding for others to consume, so we could use something like `geom_mark_hull()` from the **ggforce** package to annotate those points (in addition to adding better axis/plot titles) as shown in Figure \@ref(fig:mpg-mark-hull).
...
This simple example quickly shows how interactive web graphics can assist EDA (for another, slightly more in-depth example, see Chapter \@ref(intro-ggplotly)). And, without a doubt, `R` is one of the best and most complete environments for interactive exploration of data. Moreover, interactive web graphics can not only assist exploration, but they can also be helpful for communicating insights and preparing graphics for publication (Chapter \@ref(editing-views) has a few examples).

## What you will learn

This book provides a foundation for learning how to make interactive web-based graphics for data analysis from `R`, without assuming any prior experience with web technologies. It makes heavy use of **plotly** for rendering graphics, but also learn about other `R` packages that augment a data science workflow, such as the **tidyverse** and **shiny**. This book contains eight parts and each part contains numerous chapters:
