...
Not only do these approaches differ in static visual properties, they also differ in interactive capabilties and computational performance. That's because, the grouping approach (top panel of Figure \@ref(fig:scatter-lines)) uses just one plotly.js trace (more performant, less interactive), whereas the `color` approach (bottom panel of Figure \@ref(fig:scatter-lines)) generates one trace per line/year. In this case, the benefit of having multiple traces is that we can perform interactive filtering via the legend and compare multiple y-values at a given x. The cost of having those capabilities is that plots starts to be become sluggish after a few hundred traces, whereas thousands of lines can be rendered fairly easily in one trace. See section \@ref(scaling) for more details on scaling and performance.
...
```{r map-canada, echo = FALSE, fig.cap = "A map of Canada using the default cartesian coordinate system."}
knitr::include_graphics("images/map-canada.svg")
```
