...
```{r linked-ggally, echo = FALSE, fig.cap = "Brushing a scatterplot matrix via the `ggairs()` function in the **GGally** package. A video demonstrating the graphical queries can be viewed [here](http://i.imgur.com/dPTtH3H.gif)"}
...
```{r persistent, fig.cap = "Highlighting lines with transient versus persistent selection. In the left hand panel, transient selection (the default); and in the right hand panel, persistent selection. The video may be accessed [here](http://i.imgur.com/WyBmdv3.gif)"}
...
```{r leaflet, fig.cap = "Linking views between plotly and leaflet to explore the relation between magnitude and geographic location of earthquakes around Fiji. The video may be accessed [here](http://i.imgur.com/hd0tG0r.gif)"}
...
The interactions described thus far in [Linking views without shiny](#linking-views-without-shiny) is what @ggobi:2007 calls direct manipulation, where the user directly makes graphical queries by directly interacting with graphical elements. Sometimes it is useful to be able to define a query 
...
```{r indirect-manip, fig.cap = "Selecting cities by indirect manipulation. The video may be accessed [here]()"}
