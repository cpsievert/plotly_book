...
```{r shiny-drill-down-pie-time, echo=FALSE, fig.cap="(ref:shiny-drill-down-pie-time)"}
include_vimeo("328768848")
```

Thus far, we've only seen one level of drill-down, what about multiple levels? Introducing multiple levels adds complexity not only the implementation, but also the user experience. Especially in a drill-down approach where the _same view_ is being filtered, it can be difficult for the user to remember how they got to a particular view. Figure \@ref(fig:shiny-drill-down-pie-time-history) demonstrates how we could extend Figure \@ref(fig:shiny-drill-down-pie-time) to implement yet another level of drilling down (i.e., sub-category -> product IDs) as well as populate a `selectInput()` dropdown for each active drill-down. Not only does this help the user to remember how they got to the particular view, but it also provides the ability to easily modify the sequence of drill-down events. In terms of implementation, the main idea is the very similar to before -- we still store and update the state of each 'drill-down' in it's own reactive value, but now when a 'parent' category changes (e.g., `category`) it should invalidate (i.e., clear) any currently selected 'child' categories (e.g., `sub_category`).

```{r, eval = FALSE, summary = "Click to show/hide the code"}
```

```{r shiny-drill-down-pie-time-history, echo=FALSE, fig.cap="(ref:shiny-drill-down-pie-time)"}
