...
Numerous Figures in the following sections show how to access common **plotly** events in **shiny** and do something with the result. When using these events to inform another view of the data, it's often necessary to know what portion of data was queried in the event (i.e., the `x`/`y` positions alone may not be enough to uniquely identify the information of interest). For this reason, it's often a good idea to supply a `key` (or `customdata`) attribute, so that you can map the event data back to the original data. The `key` attribute is only supported in **shiny**, but `customdata` is officially supported by plotly.js, and thus can also be used to attach meta-information to event -- see section \@ref(javascript) for more details.
...
knitr::include_graphics("https://i.imgur.com/psFmgIw.gif")
```
...
A little known fact about **plotly** is that you can directly manipulate annotations, title, shapes (e.g., circle, lines, rectangles), legends, and more by simply adding `config(p, editable = TRUE)` to a plot `p`. Moreover, since these are all layout components, we can access information about these 'edit events', by listening to the `"plotly_relayout"` events. Figure \@ref(fig:shiny-edit-annotations) just how easy it is to access information about changes in annotation positioning and content.
...
```{r shiny-edit-annotations, echo=FALSE, fig.cap = "Accessing information about direct manipulation of annotations."}
knitr::include_graphics("https://i.imgur.com/1dZhUPM.gif")
```
...
```{r shiny-parcoords-data, echo=FALSE, fig.cap = "Displaying the highlighted observations of a parcoords trace."}
knitr::include_graphics("https://i.imgur.com/dFOUCSU.gif")
```
...
nstead of generating an entirely new view in response to a user event, sometimes you simply want to update already existing view(s). If those existing views require a lot of data, you may want to consider partially modifying the **plotly** graph (see section \@ref(proxies)), but in some cases (e.g., Figure \@ref(fig:shiny-ag)), a full re-render of the graph is responsive enough. Note how in Figure \@ref(fig:shiny-ag), when a group of states are selected, it causes the container of density plots to gray out for a split second while re-drawing both the original density (representing the entire US) and the dotted density (representing the new selection). Section \@ref(proxies) covers techniques that would allow one to avoid a full re-draw and simply add the dotted line, but it also introduces significant complexity to the implementation.
...
```{r event-priority, echo=FALSE, fig.cap = "A demo of input priority versus event priority. Clicking on the same marker repeatedly, by default, won't invalidate a reactive expression that depends on 'plotly_click', but it will invalidate when given event priority."}
knitr::include_graphics("https://i.imgur.com/QBLj2mx.gif")
```
...
```{r shiny-lmGadget, echo=FALSE, fig.cap = "Interactively removing observations from a linear model. Credit to Winston Chang for the initial implementation of this **shiny** gadget using `shiny::plotOutput()` instead of `plotly::plotlyOutput()`"}
knitr::include_graphics("https://i.imgur.com/bMkTnTG.gif")
```
...
Even in the case that you need a standalone HTML file and the R API that **plotly** provides doesn't support the type of interactivity that you desire, you can always layer on additional JavaScript to hopefully achieve the functionality you desire. This can be useful for something as simple as opening a hyperlink when clicking on marker of a plotly graph -- this topic is covered in section \@ref(javascript).