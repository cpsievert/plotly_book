...
tooltip_data <- tibble(
  ...
  text = c(
    "👋 glyphs ಠ_ಠ",
    "Hello <span style='color:red'><sup>1</sup>⁄<sub>2</sub></span> fraction",
...
```{r custom-tooltip-plotly, echo = FALSE, fig.cap = "(ref:custom-tooltip-plotly)", out.extra = 'data-url="/interactives/custom-tooltip-plotly.html"'}
...
```{r hoveron-fill-points, echo = FALSE, fig.cap = "(ref:hoveron-fill-points)", out.extra = 'data-url="/interactives/hoveron-fill-points.html"'}
...
```{r tooltip-format-boxplot, echo = FALSE, fig.cap = "(ref:tooltip-format-boxplot)", out.extra = 'data-url="/interactives/tooltip-format-boxplot.html"'}
...
```{r tooltip-format-heatmap, echo = FALSE, fig.cap = "(ref:tooltip-format-heatmap)", out.extra = 'data-url="/interactives/tooltip-format-heatmap.html"'}
...
plot_ly(x = rnorm(100, 0, 1e5)) %>%
  add_histogram(
    histnorm = "density",
    hovertemplate = "The height between <br> (%{x}) <br> is %{y:.1e} <extra>That's small!</extra>"
  )
```

```{r hovertemplate, echo = FALSE, fig.cap = "(ref:hovertemplate)", out.extra = 'data-url="/interactives/hovertemplate.html"'}
...
```{r ggplotly-text, echo = FALSE, fig.cap = "(ref:ggplotly-text)", out.extra = 'data-url="/interactives/ggplotly-text.html"'}
...
```{r ggplotly-text-tooltip, echo = FALSE, fig.cap = "(ref:ggplotly-text-tooltip)", out.extra = 'data-url="/interactives/ggplotly-text-tooltip.html"'}
...
```{r format-options, echo = FALSE, fig.cap="(ref:format-options)", out.extra = 'data-url="/interactives/format-options.html"'}
...
```{r text-clever, echo = FALSE, fig.cap="(ref:text-clever)", out.extra = 'data-url="/interactives/text-clever.html"'}
...
```{r custom-predict-text, echo = FALSE, fig.cap="(ref:custom-predict-text)", out.extra = 'data-url="/interactives/custom-predict-text.html"'}
...
```{r tooltip-style, echo = FALSE, fig.cap="(ref:tooltip-style)", out.extra = 'data-url="/interactives/tooltip-style.html"'}
...
```{r tooltip-style-ggplotly, echo = FALSE, fig.cap="(ref:tooltip-style-ggplotly)", out.extra = 'data-url="/interactives/tooltip-style-ggplotly.html"'}
...
```{r modeBarButtonsToRemove, echo = FALSE, fig.cap = "(frefmodeBarButtonsToRemove)"}
...
```{r color-plotly-js, echo = FALSE, fig.cap="(ref:color-plotly-js)", out.extra = 'data-url="/interactives/color-plotly-js.html"'}
...
```{r glyphs, echo = FALSE, fig.cap = "(ref:glyphs)", , out.extra = 'data-url="/interactives/glyphs.html"'}
...
```{r image-hyperlink, echo = FALSE, fig.cap="(ref:image-hyperlink)", out.extra = 'data-url="/interactives/image-hyperlink.html"'}
knitr::include_graphics("images/image-hyperlink.svg")
```

The approach in Figure \@ref(fig:image-hyperlink) has a downside though -- if that hyperlink breaks, then so does your plot. It'd be better to download the file to your machine and use the `dataURI()` function from the **base64enc** package (or similar) to embed the image as a data URI [@base64enc].
...
```{r image-uri, echo = FALSE, fig.cap="(ref:image-uri)", out.extra = 'data-url="/interactives/image-uri.html"'}
knitr::include_graphics("images/image-uri.svg")
```

Another approach is to convert a raster object into a data URI, which the `raster2uri()` function in **plotly** is designed to do. R actually ships with native support for raster objects and many image processing R packages either build on this data structure or provide a utility to convert to a raster object (perhaps via `as.raster()`). For example, the `readPNG()` function from the **png** package reads image data in an R array, which can be converted to a raster object [@png].
...
```{r raster, echo = FALSE, fig.cap = "(ref:raster)", out.extra = 'data-url="/interactives/raster.html"'}
...
```{r localization, echo = FALSE, fig.cap="(ref:localization)", out.extra = 'data-url="/interactives/localization.html"'}
...
```{r mathjax, echo = FALSE, fig.cap="(ref:mathjax)", out.extra = 'data-url="/interactives/mathjax.html"'}
knitr::include_graphics("images/mathjax.psv")
```
...
```{r mathjax2, echo = FALSE, fig.cap="(ref:mathjax2)", out.extra = 'data-url="/interactives/mathjax2.html"'}
...
As Chapter \@ref(intro-plotly) first introduced, we can express multi-layer **plotly** graphs as a sequence (or, more specifically, a directed acyclic graph) of **dplyr** data manipulations and mappings to visuals. For example, to create Figure \@ref(fig:houston2), we could group `txhousing` by `city` to ensure the first layer of `add_lines()` draws a different line for each city, then `filter()` down to Houston so that the second call to `add_lines()` draws only Houston. 
...
```{r houston2, echo = FALSE, fig.cap = "(ref:houston2)", out.extra = 'data-url="/interactives/houston2.html"'}
...
```{r houston-vs-sa, echo = FALSE, fig.cap = "(ref:houston-vs-sa)", out.extra = 'data-url="/interactives/houston-vs-sa.html"'}
...
```{r summary, echo = FALSE, fig.cap = "(ref:summary)", out.extra = 'data-url="/interactives/summary.html"'}
...
```{r forecast, echo = FALSE, fig.cap = "(ref:forecast)", out.extra = 'data-url="/interactives/forecast.html"'}
...
* `layout()` for modifying aspects of the layout, which can be used to:
...
In addition to using the functions above to modify `ggplotly()`'s return value, the `ggplotly()` function itself provides some arguments for controling that return value. In this chapter, we'll see a couple of them:

* `dynamicTicks`: should plotly.js dynamically generate axis tick labels? Dynamic ticks are useful for updating ticks in response to zoom/pan interactions; however, they can not always reproduce labels as they would appear in the static ggplot2 image (see Figure \@ref(fig:ggplotly-rangeslider)).
* `layerData`: return a reference to which **ggplot2** layer's data (see Figure \@ref(fig:se-annotations))?

## Modifying layout

Any aspect of a **plotly** objects layout can be modified^[Or, in the case of cumulative attributes, like `shapes`, `images`, `annotations`, etc, these items will be added to the existing items) via the `layout()` function. By default, since it doesn't always make sense to compare values, `ggplotly()` will usually set `layout.hovermode='closest'`. As shown in Figure \@ref(fig:ggplotly-rangeslider), when we have multiple y-values of interest at a specific x-value, it can be helpful to set `layout.hovermode='x'`. Moreover, for a long time series graph like this, zooming in on the x-axis can be useful -- `dynamicTicks` allows plotly.js to handle the generation of axis ticks and the `rangeslider()` allows us to zoom on the x-axis without losing the global context.
...
```{r ggplotly-rangeslider, echo = FALSE, fig.cap = "(ref:ggplotly-rangeslider)", out.extra = 'data-url="/interactives/ggplotly-rangeslider.html"'}
...
```{r simple-scatterplot, echo=FALSE, fig.cap="(ref:simple-scatterplot)", out.extra = 'data-url="/interactives/simple-scatterplot.html"'}
...
```{r listviewer, echo = FALSE, fig.cap = "(ref:listviewer)", out.extra = 'data-url="/interactives/listviewer.html"'}
...
```{r style-hoverinfo, echo = FALSE, fig.cap = "(ref:style-hoverinfo)", out.extra = 'data-url="/interactives/style-hoverinfo.html"'}
...
```{r se-annotations, echo = FALSE, fig.cap = "(ref:se-annotations)", out.extra = 'data-url="/interactives/se-annotations.html"'}
...
```{r StatBin, echo = FALSE, fig.cap = "(ref:StatBin)", out.extra = 'data-url="/interactives/StatBin.html"'}
...
```{r StatDensity2d, echo = FALSE, fig.cap = "(ref:StatDensity2d)", out.extra = 'data-url="/interactives/StatDensity2d.html"'}
...
```{r xspline, echo = FALSE, fig.cap = "(ref:xspline)", out.extra = 'data-url="/interactives/xspline.html"'}
