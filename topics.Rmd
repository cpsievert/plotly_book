...
```{r houston2, echo = FALSE, fig.cap = "Monthly median house prices over time for 46 Texan cities (in blue). Houston is highlighted in orange."}
...
```{r houston-vs-sa, echo = FALSE, fig.cap = "Monthly median house price in Houston and San Antonio in comparison to other Texan cities."}
...
```{r summary, echo = FALSE, fig.cap = "First, second, and third quartile of median monthly house price in Texas."}
...
```{r forecast, echo = FALSE, fig.cap = "Layering on a 4-year forecast from a exponential smoothing state space model."}
...
```{r ggplotly-rangeslider, echo = FALSE, fig.cap = "Adding `dynamicTicks`, a `rangeslider()`, and a comparison hovermode to improve the interactive experience of a `ggplotly()` graph."}
...
```{r simple-scatterplot, fig.cap="A scatterplot with a fitted line and confidence band."}
...
```{r style-hoverinfo, echo = FALSE, fig.cap = "Using the `style()` function to modify hoverinfo attribute values of a plotly object created via `ggplotly()` (by default, `ggplotly()` displays hoverinfo for all traces). In this case, the hoverinfo for a fitted line and error bounds are hidden."}
...
```{r se-annotations, echo = FALSE, fig.cap = "Leveraging data associated with a `geom_smooth()` layer to display additional information about the model fit."}
...
```{r StatBin, echo = FALSE, fig.cap = "Leveraging output from `StatBin` to add annotations to a stacked bar chart (created via `geom_bar()`) which makes it easier to compare bar heights."}
...
```{r StatDensity2d, echo = FALSE, fig.cap = "Leveraging output from `StatDensity2d` to add annotations to contour levels."}
...
```{r image-hyperlink, echo = FALSE, fig.cap="Embedding an image using a hyperlink. This example uses paper coordinates (i.e., normalized 0-1 scale relative to the graph area) to place the image just above the graph."}
...
```{r image-uri, echo = FALSE, fig.cap="Embedding an image using a data URI. This approach ensures your image stays embedded in the plot."}
...
```{r raster, echo = FALSE, fig.cap = "Reading a png image into R with `png::readPNG()`, converting the RBGA array into a raster object, generating a data URI of the raster via `raster2uri()`, then embedding the URI as an image in a **plotly** graph."}
