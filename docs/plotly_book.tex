...
\index{plot\_ly()@\texttt{plot\_ly()}!Introduction}
...
\index{ggplotly()@\texttt{ggplotly()}!Introduction}
...
\index{ggplotly()@\texttt{ggplotly()}!ggforce}
...
\index{ggplotly()@\texttt{ggplotly()}!GGally}
...
\index{ggplotly()@\texttt{ggplotly()}!naniar}
...
\index{
  plot\_mapbox()@\texttt{plot\_mapbox()}
}
...
In addition to scatter traces, both of the integrated mapping solutions (i.e., \texttt{plot\_mapbox()} and \texttt{plot\_geo()}) have an optimized choropleth trace type (i.e., the \href{https://plot.ly/r/reference/\#choroplethmapbox}{choroplethmapbox} and \href{https://plot.ly/r/reference/\#choropleth}{choropleth} trace types). The choropleth trace currently accepts three different \texttt{locationmode}s: either \texttt{"ISO-3"}, \texttt{"USA-states}, or \texttt{"country-names"}.
...
\StringTok{  }\KeywordTok{add_trace}\NormalTok{(}
    ...
    \CommentTok{# See how this GeoJSON URL was generated, go to}
