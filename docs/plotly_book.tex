...
\CommentTok{#>  crayon         1.3.4      2019-04-03 [1]}
\CommentTok{#>  crosstalk      1.0.1      2019-04-11 [1]}
...
\CommentTok{#>  plotly         4.9.0      2019-04-18 [1]}
...
\CommentTok{#>  CRAN (R 3.5.2)                }
\CommentTok{#>  CRAN (R 3.5.2)                }
\CommentTok{#>  Github (r-lib/crayon@84be620) }
\CommentTok{#>  local                         }
\CommentTok{#>  CRAN (R 3.5.2)                }
...
\CommentTok{#>  CRAN (R 3.5.1)                }
\CommentTok{#>  CRAN (R 3.5.0)                }
\CommentTok{#>  Github (r-lib/later@267deed)  }
\CommentTok{#>  CRAN (R 3.5.3)                }
\CommentTok{#>  CRAN (R 3.5.2)                }
\CommentTok{#>  CRAN (R 3.5.0)                }
\CommentTok{#>  CRAN (R 3.5.3)                }
...
\CommentTok{#>  CRAN (R 3.5.0)                }
\CommentTok{#>  CRAN (R 3.5.3)                }
\CommentTok{#>  Github (r-lib/scales@c374014) }
\CommentTok{#>  local                         }
\CommentTok{#>  CRAN (R 3.5.0)                }
\CommentTok{#>  CRAN (R 3.5.2)                }
\CommentTok{#>  CRAN (R 3.5.2)                }
...
\caption{Using the integrated orthographic projection to visualize flight patterns on a `3D' globe. For a video demonstration of the interactive, see \url{https://vimeo.com/317358033}}\label{fig:geo-flights}
\end{figure}
...
\KeywordTok{data}\NormalTok{(sales, }\DataTypeTok{package =} \StringTok{"plotlyBook"}\NormalTok{)}
...
\NormalTok{  \})}
  ...
  \CommentTok{# with the click data, but we can leverage `customdata` as a workaround}
...
\StringTok{      }\KeywordTok{add_pie}\NormalTok{(}\DataTypeTok{labels =} \OperatorTok{~}\NormalTok{labels, }\DataTypeTok{values =} \OperatorTok{~}\NormalTok{values, }\DataTypeTok{customdata =} \OperatorTok{~}\NormalTok{labels) }\OperatorTok{%>%}
...
\NormalTok{  \})}
  ...
  \CommentTok{# update the current category if the clicked value matches a category}
  \KeywordTok{observe}\NormalTok{(\{}
...
\caption{A drill-down pie chart of sales by category. By clicking on a category (e.g., Furniture), the pie chart updates to display sales by sub-categories within Furniture. For a video demonstration of the interactive, see \url{https://vimeo.com/328763167}}\label{fig:shiny-drill-down-pie}
\end{figure}
...
\caption{Coordinating drill-down across in multiple views. By clicking on a category (e.g., Technology), both the bar chart and the time-series updates to display sales within Technology. For a video demonstration of the interactive, see \url{https://vimeo.com/328768848}}\label{fig:shiny-drill-down-bar-time}
\end{figure}
...
\NormalTok{  output}\OperatorTok{$}\NormalTok{history <-}\StringTok{ }\KeywordTok{renderUI}\NormalTok{(\{}
    \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\KeywordTok{length}\NormalTok{(drills}\OperatorTok{$}\NormalTok{category)) }\KeywordTok{return}\NormalTok{(}\StringTok{"Click the bar chart to drilldown"}\NormalTok{)}
...
\item
  JS event handlers can be noticably faster than running comparable code on an external R process, especially with a slow internet connection.
\end{enumerate}
...
\StringTok{  }\KeywordTok{add_histogram}\NormalTok{(}
    ...
    \DataTypeTok{hovertemplate =} \StringTok{"The height between <br> (%\{x\}) <br> is %\{y:.1e\} <extra>That's small!</extra>"}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#> \}}
\CommentTok{#> <bytecode: 0x7fefe455e508>}
...
\CommentTok{#>     prefix_class(data, "GeomPath")}
\CommentTok{#> \}}
\CommentTok{#> <bytecode: 0x7fefe74df8f0>}
\CommentTok{#> <environment: namespace:plotly>}
\end{Highlighting}
\end{Shaded}
