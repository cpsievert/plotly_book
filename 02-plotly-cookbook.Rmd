...
Visualizing multiple discrete distributions is suprisingly complicated -- evidenced by the multiple books written on the subject . When dealing with multiple categorical variables, perhaps the easiest thing to do is visualize counts of their interaction, as Figure \@ref(fig:cut-by-clarity) does using a grouped bar chart. 

```{r cut-by-clarity, fig.cap = "A grouped bar chart"}
p <- plot_ly(diamonds, x = ~cut, color = ~clarity)
```

```{r}
subplot(
  add_histogram(p),
  add_histogram(p, histnorm = "percent"),
  nrows = 2, shareX = TRUE
) %>% hide_legend()
```


```{r many-clarity-stacked, fig.cap = "A stacked bar chart"}
plot_ly(diamonds, x = ~cut, color = ~clarity) %>% 
  add_histogram() %>% layout(barmode = "stack")
```


```{r many-clarity-cut, fig.width = 8, fig.cap = "A trellis display of diamond price by diamond color."}
one_plot <- function(d) {
  plot_ly(d, x = ~cut, color = ~color) %>%
    add_annotations(
      ~paste("Clarity:", unique(clarity)), x = 0.5, y = 1, 
      xref = "paper", yref = "paper", showarrow = FALSE
    )
...
p <- ggplot(data = happy) +
  ...
  labs(x = " Sex : Health", y = "Happiness", title = "divider = mosaic()")
ggplotly(p)
```
...
### Multi-variate numerical distributions

Visualizing many numerical distributions is a bit more tricky. 
