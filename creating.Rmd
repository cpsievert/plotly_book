...
```{r color-mapping, echo = FALSE, fig.cap = "Mapping data values to a visual color range.", out.width = "35%"}
...
The sections that follow in this chapter demonstrate various type of data views that are powered by the scatter trace.
...
A common problem with scatterplots is overplotting, meaning that there are multiple observations occupying the same (or similar) x/y locations. Figure \@ref(fig:scatterplots) demonstrates one way to combat overplotting via alpha blending. When dealing with tens of thousands of points (or more), you should consider using `toWebGL()` to render plots using Canvas rather than SVG (more in section \@ref(scaling)), or performing 2D density estimation (section \@ref(rectangular-binning-in-R)).
...
```{r scatterplots, fig.cap = "Combating overplotting in a scatterplot with alpha blending."}
knitr::include_graphics("images/scatterplots")
```
