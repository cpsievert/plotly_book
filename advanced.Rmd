...
## Custom behavior via JavaScript {#javascript}
...
d <- tibble::tibble(
  id = seq_len(nagents),
  first = runif(nagents, Sys.time() - 1e5, Sys.time()),
  second = runif(nagents, first, first + 1e5),
  third = runif(nagents, second, second + 1e5),
  meta = paste("Some metainfo about case ID:", id)
)

# how the selected markers should appear
...
gather(d, variable, value, -id, -meta) %>%
  mutate(value = anytime(value)) %>%
  highlight_key(~id) %>%
  plot_ly(x = ~value, y = ~factor(variable, levels = lvls)) %>%
  group_by(id) %>%
  add_trace(
    type = "scatter",
    mode = "markers+lines",
    alpha = 0.4,
    text = ~paste("Case id:", id),
    hoverinfo = "x+text",
    customdata = ~meta
  ) %>%
  toWebGL() %>%
  ...
  highlight(
    ...
    selected = attrs_selected(marker = marker_selected, line = list(color = "black"))
  ) %>%
  htmlwidgets::onRender("
    function(el, x) {
      var gd = document.getElementById(el.id);
      gd.on('plotly_hover', function(ed) {
        var ann = {
           text: ed.points[0].customdata,
           x: 0.5,
           y: 1,
           xref: 'paper',
           yref: 'paper',
      ...
      gd.on('plotly_doubleclick', function(ed) {
